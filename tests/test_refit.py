...
def test_the_whole_ledger_recomputes_to_ninety_seven_refits(ledger: MemoryLedger):
    plan = ledger.executable()

    assert len(plan.members) == 33
    assert sum(len(member.budgets) for member in plan.members) == 97
