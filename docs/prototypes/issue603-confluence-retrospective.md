...
12개 생활 습관 변수로 스마트폰 중독 위험의 순서를 예측했습니다.
...
첫 기준 실행은 12개 원시 변수를 그대로 사용했습니다.
점수 자체보다 중요한 목적은 이후 실험이 돌아올 비교 기준을 하나 세우는 일이었습니다.

처음 크게 오른 순간은 복잡한 모형을 새로 붙였을 때가 아니었습니다.
같은 숫자를 연속적인 크기로도 보여 주고 정확한 값의 범주로도 보여 주자, 내부 단일 구성 점수가 `0.96605`로 올랐습니다.

반대로 숫자를 모두 범주로만 바꾸자 AUC 차이가 `-0.00417`이었습니다.
좋은 결과만 선으로 남기면 순조로워 보이지만, 실제 다음 선택을 정해 준 것은 이런 중단 기록이었습니다.
...
모형의 한계라고 결론 내리기 전에 자료가 실제로 어떻게 전달되는지 확인했고, 범주값을 이름표에 대응시키기 전에 자료형이 바뀌면서 값이 대량으로 미등록 처리되는 문제를 찾았습니다.
...
마지막에는 자체 예측 36개와 공개 예측 278개를 합친 314개 예측 열로 내부 결합 점수 `0.97038`을 만들었습니다.
