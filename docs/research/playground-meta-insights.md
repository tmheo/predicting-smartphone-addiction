...
원본 데이터(S6E8에서는 7,500행 프록시 사본)에서 피처 값별 타깃 통계를 미리 계산해 두고, 그것을 합성 train/test에 새 컬럼으로 붙이는 기법이다.
예를 들어 원본에서 sleep_hours가 6.5인 행들의 addicted 비율이 0.82였다면, 합성 데이터의 sleep_hours가 6.5인 모든 행에 그 0.82를 피처 값으로 준다.
prior(사전 정보)라 부르는 이유는 합성 train의 라벨이 아니라 원본이라는 외부 소스에서 가져온 사전 지식이기 때문이다.
행 주입(3.1)과의 결정적 차이는 모델이 여전히 합성 train으로만 학습한다는 점이다.
행 주입은 원본 행이 train에 들어가 모델이 원본 분포 자체를 학습하므로 분포가 다르면 독이 되지만, 새 컬럼은 원본이 피처 값 하나로만 참여하므로 그 피처가 유용한지를 모델이 합성 데이터 기준으로 스스로 판단하고, 분포가 달라도 무시하면 그만이다.
누수도 원리적으로 0이다: 통계가 원본의 라벨에서만 나오고 합성 train의 라벨을 전혀 쓰지 않으므로, 일반 타깃 인코딩과 달리 fold-fit 없이 전체에 한 번 붙여도 검증 폴드의 라벨이 새어들 통로 자체가 없다.
...
부스팅 모델은 예측을 공통 초기값(로짓 0)에서 시작해 트리를 하나씩 더하며 보정해 나가는데, base_margin(XGBoost 용어, LightGBM에서는 init_score)은 이 출발점을 행마다 다른 값으로 지정하는 파라미터다.
출발점을 주면 경사(gradient)가 그 출발점 기준으로 계산되므로, 트리들은 출발점이 이미 맞힌 부분은 건드리지 않고 출발점과 실제 라벨의 차이(잔차)만 학습한다.
S6E8 적용 절차는 이렇다.
1단계로 원본 프록시 7,500행으로만 학습한 모델이나 이미 알려진 임계값 규칙을 준비하고, 그것으로 합성 train/test의 모든 행을 예측만 해서 확률 p를 얻어 로짓 log(p/(1-p))으로 변환한다.
합성 데이터는 1단계에서 입력(피처)으로만 쓰이고, 합성 train의 라벨은 1단계가 전혀 보지 않는다.
2단계로 합성 train을 학습할 때 그 로짓을 base_margin/init_score로 넣으면, 모델이 "원본 규칙은 이렇게 말한다"에서 출발하므로 학습 대상이 원본 규칙이 설명하지 못하는 부분, 즉 생성기의 완만한 확률 구조만 남는다.
예측 시에도 test의 1단계 로짓을 똑같이 공급해야 하며(최종 로짓 = 1단계 로짓 + 2단계 트리 보정의 합), 학습 때만 넣고 예측 때 빼먹는 것이 흔한 실수다.
1단계가 합성 train의 라벨을 본 적이 없으므로 3.2와 같은 논리로 fold-fit 없이도 누수가 0이다.
단 아래 변형처럼 1단계를 합성 train으로 학습한 다른 모델들의 예측으로 대체하는 경우에는 반드시 OOF 예측을 써야 한다.
3.2와의 관계는 같은 원천(원본의 타깃 정보)을 다른 통로로 주입하는 것이다.
3.2는 피처 값별 원본 통계를 새 컬럼으로 주고 모델이 알아서 쓰게 하는 방식이고, 3.3은 원본 예측기의 행별 로짓을 부스팅의 출발점으로 강제하는 방식이다.
...
세 기법의 공통 아이디어는 합성 행 하나하나가 원본 데이터와 얼마나 가깝거나 겹치는지를 측정해 피처로 주는 것이다.
3.2가 원본의 라벨 통계를, 3.3이 원본 예측기의 로짓을 주입한다면, 3.4는 라벨 없이 원본의 값 자체와의 거리와 일치 여부를 주입한다.
...
- S6E3 1위의 최고 효과 기법은 합성 값을 원본 최근접 값으로 되돌린 snap과 이탈 크기 snap_diff였다 ([S6E3 1위 writeup](https://www.kaggle.com/competitions/playground-series-s6e3/writeups/1st-place-gpt5-4-gemini3-1-claudeopus4-6-kgm)).
  합성 생성기는 원본 값을 흉내 내다가 조금씩 어긋난 값을 만들 수 있고, 그 이탈 크기가 "생성기가 이 행을 만들 때 원본에서 얼마나 벗어났는가"라는 생성기 신호를 담기 때문에 통한 기법이다.
  그러나 S6E8은 생성기가 눈금값 패턴을 그대로 복제했으므로 값 대부분이 이미 원본 눈금 위에 앉아 있고, 그렇다면 snap은 아무것도 바꾸지 않는 변환(항등)이 되고 snap_diff는 전부 0이 된다.
  따라서 눈금 이탈 행 비율 측정을 선행 조건으로 하고, 그 비율이 사실상 0이면 건너뛰는 조건부 트랙이다.
- S4E1 2위는 1~10개 피처 부분집합의 값 조합이 원본에 그대로 존재하는지를 0/1 피처로 넣어 극적인 타깃 분리(0.203 대 0.708)를 얻었다 ([S4E1 2위](https://www.kaggle.com/competitions/playground-series-s4e1/discussion/472496)).
  생성기가 원본 조합을 통째로 복제한 행과 새로 지어낸 행은 라벨이 만들어진 방식이 달랐다는 뜻이다.
  S6E8은 원본 프록시가 7,500행뿐이라 컬럼을 많이 묶을수록 조합의 경우의 수가 폭발해 "원본에 존재"가 거의 전부 0이 되므로, 2~3개 컬럼 조합까지만 유효할 것이다.
  또 이미 검증된 정확값 TE가 "같은 눈금값에서 나온 행 묶음" 정보를 상당 부분 잡고 있으므로, 이 피처가 그 위에 새로 보태는 것이 있는지 기여 겹침 확인이 필수다.
- 원본 cKDTree 최근접 이웃 라벨 조회와 원본 전용 PCA/DAE 피처는 구현이 저렴한 보조 다양성 재료다 ([S6E3 1위 writeup](https://www.kaggle.com/competitions/playground-series-s6e3/writeups/1st-place-gpt5-4-gemini3-1-claudeopus4-6-kgm)).
  전자는 합성 행마다 피처 공간에서 가장 가까운 원본 행을 찾아(cKDTree는 scipy의 빠른 최근접 탐색 자료구조) 그 이웃의 라벨이나 이웃까지의 거리를 피처로 주는 것으로, "이 합성 행과 가장 닮은 원본 행은 addicted였는가"를 묻는 셈이다.
  후자는 원본으로만 학습한 PCA나 잡음 제거 오토인코더(DAE)에 합성 행을 통과시켜, 원본의 구조를 기준 좌표계로 삼은 저차원 표현을 피처로 얻는 것이다.
  둘 다 단독으로 순위를 가를 기대는 없고, 기존 모델과 다른 관점을 주는 스태킹 다양성 재료로 분류된다.
