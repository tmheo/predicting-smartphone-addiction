...
- 이 추정기는 완료 에피소드로 보정하지 않았다.
  S6 완료 7개 중 AUC 3개 에피소드의 마감 시점 public LB로 같은 스윕을 계산해 전멸(S6E2)과 유지(나머지 2개)가 이 추정기로 사전 분리되는지 확인하는 것이 자연스러운 후속이지만, 에피소드별 채점 표본 크기와 양성 비율 확보가 필요해 이 이슈 범위 밖으로 남긴다.
