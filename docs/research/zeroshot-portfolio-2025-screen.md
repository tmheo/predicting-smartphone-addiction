...
run `d92c47c8d09f45479091e2dd17a45c1f`(--stage confirm, 시드 42·43·44), 기록 커밋 `75452bb`.

| 항목 | 값 |
| --- | ---: |
| 3시드 평균본 OOF AUC | 0.9687158156 |
| 시드별 OOF | 42: 0.9686372328, 43: 0.9686247205, 44: 0.9686435748 |
| fold별(평균본) | 0.96809, 0.96881, 0.96882, 0.96935, 0.96852 |
| 최근접 풀 구성원 | exp074 스피어만 `0.99829` (다음 exp045 `0.99651`) |

- 같은 계열 풀 최고 exp074(0.96840) 대비 +0.00032, 트리 전체 최고 exp070(0.96858) 대비 +0.00014로, 진입 시 Lookup 두 판 다음의 트리 최고 구성원이 된다.
- 최근접 exp074와 0.99829는 중복 게이트(0.998) 초과다.
  ADR 0001 계열 2의 규칙대로면 성능 높은 exp117이 exp074를 교체한다.
  exp074의 kitopl 자릿수 특성 축이 풀에서 빠지는 결정이므로 #187 재평가에 이 사실을 전달한다.
- exp119의 3시드 확정은 취소했다.
  exp117 확정이 성립했고, exp119는 exp117과 스피어만 0.99890이라 어느 경우든 공존할 수 없는 예비였다.
  seed 42 스크리닝 실행과 묶음은 증거로 남긴다(로컬 부하 경합으로 취소 시점까지 15개 학습 중 6개 완료, MLflow 상태 KILLED).

## 다음 단계

1. 실행 기록 묶음 5개(`run-logs/bundles/`: 스크리닝 4건 + exp117 confirm)를 본 체크아웃에 반입(pipeline.bundle import)한 뒤, exp117 confirm run으로 ADR 0001 계열 2의 진입 판정(pipeline.pool --admit)을 수행한다.
2. 보강된 풀의 nested 재평가와 결합 전략 재선정은 #187이 소유한다.
3. 중단 조건 판정: 2023년판 100설정 확장은 열지 않는다.
