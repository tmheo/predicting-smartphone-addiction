...
`schedule`로 시작한 실행의 최근 성공 여부는 운영 상태를 관찰하는 증거로 기록하되 개별 유료 작업의 시작 관문으로 쓰지 않는다.
GitHub 예약 실행은 지연되거나 누락될 수 있으므로 최근 성공을 요구하면 안전장치가 실험 시작을 불필요하게 막고, 최근 성공이 앞으로의 삭제를 보장하지도 않는다.
개별 작업의 시작 관문은 그 작업의 정확한 대상을 등록하고 `observe` 방식으로 다시 읽는 확인이다.
