...
3. 작업 실행.
   ...
   `baseline k`(비교 팔 313의 봉인 분할 예측 5개, 분할당 14분 안팎)와 `ladder <구성>`(구성 하나의 nested 5분할, 300열대는 70분 안팎)이 하위 프로세스로 돈다.
   결합기는 #498 개정 뒤 규제 강도 C와 λ를 안쪽에서 함께 고르는 `CSelectedShrunkRankLogitCombiner`라 분할당 로지스틱 적합이 29회다.
...
4. 비교와 선택.
   ...
   비교 팔 자기 검사(313 봉인 예측을 고정 결합기 `CSelectedShrunkRankLogitCombiner`로 다시 만들어 이어붙인 AUC와 분할별 AUC가 #489 규제 강도 선택판 `30b6f97c`의 `0.9703609`와 잡음 바닥 `5.7e-06` 안인지, #498 개정), 구성별 nested AUC·가중 OOF AUC(진단)·313 대비 이어붙인 차이·분할별 차이·5/5 부호·문턱 판정, 선택 규칙 적용 경로와 제안 구성.
   자기 검사가 실패하면 판정 불가다.
   통과 구성이 없으면 현재 두 장(`e88f706e` + `30b6f97c`) 유지가 결론이다.
...
- 비교 팔 313의 봉인 분할 예측은 분할당 14분 안팎(#489 실측 796~853초, 최대 메모리 14GB).
- 사다리 구성 하나(313 + 후보 수 열)의 nested 5분할은 분할당 14분 기준으로 70분 안팎이다.
  전체 구성 작업은 근접 중복 진단(열린 4분할 스피어만 5회)이 몇 분 더 든다.
- 후보 8개·6구성이면 작업 11개, 동시 3개로 세 시간 안팎이다.
- 08-30 2회차의 비교 팔은 #489 규제 강도 선택판 `30b6f97c`이며 판정 도구가 그 기준값(`docs/research/logistic-c-selection/issue489/comparison.json`)을 읽는다.
