...
여러 모델의 예측을 앙상블하기 시작하면 OOF AUC만으로는 평가가 되지 않습니다.
어떤 예측을 앙상블에 넣고 어떻게 합칠지를 OOF 점수를 보면서 고르면, 그 선택 자체가 정답을 엿본 셈이 되기 때문입니다.
...
3. 그 행들 위에서 어떤 모델의 예측을 넣을지 고르고, 합치는 가중치를 맞춥니다.
4. 고른 구성과 가중치를 fold 3 행의 OOF 예측에 적용해 fold 3의 앙상블 예측을 얻습니다.
...
일찍 멈춘 탐색도 그만큼 많았습니다.
Lookup-Transformer의 학습률과 학습 일정, 최적화 알고리즘을 바꾼 모델 설정 17개는 fold 0 기준을 모두 넘지 못해서 전체 실행을 열지 않았습니다.
