...
병합한 커밋을 기본 작업 폴더에서 확인할 때는 다음과 같이 격리 검증을 실행한다.

```bash
scripts/verify_environment_gates.sh --isolated
```

격리 검증은 현재 작업 폴더가 깨끗한지 확인한 뒤 현재 커밋의 임시 Git 작업 폴더를 만든다.
선언된 입력은 원래 작업 폴더에서 해시를 확인해 임시 작업 폴더로 복사하므로, 원래 `data/`에 쓰기 가능한 입력이나 목록 밖 연구 자료가 있어도 변경하거나 삭제하지 않는다.
모든 환경 관문을 마치면 성공 여부와 관계없이 임시 작업 폴더를 정리한다.
커밋되지 않은 변경은 검증 대상에 포함되지 않으므로 격리 검증을 시작하지 않는다.
